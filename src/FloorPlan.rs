...
struct FloorPlan {
    ...
    height: u32,
    tolerance: u32,
    dominate_pixel: Rgba<u8>,
}

impl FloorPlan {
    pub fn new(path: &str, tolerance: u32) -> FloorPlan {
        ...
        for y in 0..height {
            ...
            if found_at.0 < u32::MAX {
                ...
                while x > found_at.0 as u32 {
                    if has_n_bellow_with_tolerance(img, dominate_pixel, width, height, x, y, 5, upset) {
                        count += 1;
                        if count > 5 {
                            points.1.1 = x + count;
                            x_wall.push(points);
    ...
    has_n_pixel_infront_with_tolerence(&self){
